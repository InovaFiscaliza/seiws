...
from seiws.config import CHAVES_API
...
def test_listar_unidades_valid(mock_zeep_client):
    ...
    response = client.listar_unidades(id_tipo_procedimento="123", id_serie="456")
...
def test_listar_unidades_edge_cases(mock_zeep_client):
    ...
    response = client.listar_unidades(id_tipo_procedimento="", id_serie="")
    assert response == mock_response

    # Edge case: very long strings
    long_string = "x" * 1000
    response = client.listar_unidades(
        id_tipo_procedimento=long_string, id_serie=long_string
    )
    assert response == mock_response
